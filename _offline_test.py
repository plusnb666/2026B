# -*- coding: utf-8 -*-
"""问题3 离线端到端测试（临时）：本地起一个假模拟器 HTTP 服务，
按附件2 规则模拟 12 个全向干扰源（±1°示向误差、虚拟时间规则），
跑 robot.py 完整策略，验证能否全部清除。
"""
import io
import contextlib
import json
...
import robot  # noqa: F401  导入即加载策略模块（不执行 main）
...
def _error(x, y, ch):
    ...
    if key not in _err_cache:
        _err_cache[key] = rng.uniform(-1.0, 1.0)
...
def run_case(n, seed):
    """跑一个随机场景，返回 (清除数, 总数, 虚拟时间)"""
    global sources, state, rng, _err_cache
    ...
    # 注意：2026 端口可能被真模拟器占用，离线测试用 20262
    server = ThreadingHTTPServer(("127.0.0.1", 20262), Handler)
    t = threading.Thread(target=server.serve_forever, daemon=True)
    t.start()

    client = robot.RobotClient(base_url="http://127.0.0.1:20262", robot_id="TEST")
    strategy = robot.RobotStrategy(client)
    resp = client.enter()
